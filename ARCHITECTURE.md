...
## 现状(已建)
五层贯通 + 产线 + 扮演页(流式输出+中止 · 多 provider 容错+超时 · 作者笔记 · 快速回复 · 多开场白 · 任意楼编辑/删/重生 · 后台自动总结) + 群聊 + 线分支 + 模式层 + 枢纽穿梭 + 多模态 + 导入/融合 + 网页控制台 + 设置热加载。**测试全套绿**(套数 / 引擎模块数 / MCP 工具数以 `python -m sim.run_tests`、`sv doctor` 实际输出为准,文档不写死)。
