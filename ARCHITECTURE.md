...
- **`sv/mcp_server.py` 零依赖 stdio MCP**(**40 个 typed 工具**,翻译成 CLI argv 复用,零回归)——作 OpenClaw/Hermes skill 时用,生成走宿主模型。
...
## 现状(已建)
五层贯通 + 产线 + 扮演页 + 枢纽穿梭 + 多模态 + 导入/融合 + 网页控制台 + 设置热加载。**测试 15 套全绿**(memory/forge/lenses/nexus/llm/narrate/codex/manage/render/config/checks/hooks/import/chat/smoke)。25 个引擎模块、40 个 MCP 工具。

## Backlog(后面再做 · 已记)
| 项 | 说明 | 状态 |
|----|------|------|
| 立绘表情切换 | 多张情绪立绘按回复情绪换(扮演页) | 待做(挂在扮演页) |
| 文生图入对话 | 对话中一键出场景图(render 已有,接进 chat) | 待做 |
| 世界书预设 | SillyTavern preset 的简化版(prompt 组合/采样) | 待做 |
| 批量造世界 | 选题材+元件一次生成一批副本 | 用户暂缓 |
| simulate 自演化 | 开世界/角色自主跑 | 用户暂缓(能力已建,默认关) |
| 向量记忆 | 语义召回(EMBED_PROVIDER 接口已留) | 规模驱动才上 |
| 线分支/存档 | 平行可能性(entity 现 world 级共享,需 per-thread 叠加) | 设计待定 |
