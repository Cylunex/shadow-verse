"""暗宇宙 · 网页路由处理器 —— 从 webapp.py 按「薄路由」原则拆出(纯搬运,零行为变化)。

GET/POST 的全部 api_*/post_* 处理器 + 路由表 GET_ROUTES/POST_ROUTES 都在这里;
webapp.py 只留 HTTP 壳(Handler)+ 静态/图片服务 + main()。处理器逻辑逐字未改。
"""
